return
{
    ...
    scheme     = { cmd = {[[guile "$file"]], [[mit-scheme --quiet --load "$file" --eval "(exit)"]], [[csi -s "$file"]], [[racket "$file"]]}},
    lisp       = { cmd = {[[sbcl --script "$file"]], [[clisp "$file"]], [[ecl -shell "$file"]], [[ros run -s "$file"]]}}
}
